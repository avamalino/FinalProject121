local cpml_mat4  = require(... .. '.minimal-mat4')
...
function anim:stop(track)
	if track ~= nil then
		assert(self.timeline[track], "anim9: Cannot stop track, track doesn't exist!")
	end
...
function anim:transition(track, length)
	assert(track, "anim9: Cannot transition, track does not exist!")
