...
local function findClosest(self, verts, func, ...)
    ...
    local translation_x = self.translation.x
    local translation_y = self.translation.y
    local translation_z = self.translation.z
    local scale_x = 1--self.scale[1]
    local scale_y = 1--self.scale[2]
    local scale_z = 1--self.scale[3]
...
function collisions:rayIntersection(src_x, src_y, src_z, dir_x, dir_y, dir_z)
    return findClosest(self, self.collider, triangleRay, src_x, src_y, src_z, dir_x, dir_y, dir_z)
end

function collisions:sphereIntersection(src_x, src_y, src_z, radius)
    return findClosest(self, self.collider, triangleSphere, src_x, src_y, src_z, radius)
end
...
function collisions:generateAABB()
    ...
    for _,vert in ipairs(self.collider) do
